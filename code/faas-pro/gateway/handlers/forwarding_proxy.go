...
var chainMatcher = regexp.MustCompile("^/?(?:async-)?chain/([^/?]+)([^?]*)")
...
func GetFunctionName(requestURL string) string{
	params1 := strings.Split(requestURL, "?")
	params2 := strings.Split(params1[0], "/")
	return "faas_function=" + params2[len(params2)-1]
}
...
// 		// 随机选取目标节点
// 		functionLabel := GetFunctionName(requestURL)
// 		pods, err := proxy.Clientset.CoreV1().Pods("openfaas-fn-zhy").List(context.TODO(), metav1.ListOptions{LabelSelector: functionLabel})
// 		var HostIP string
// 		if len(pods.Items) == 0 {
// 			log.Printf("没找到对应Pod")
// 			HostIP = RandMapKey(proxy.NodeNetesURL)
// 		}else{
// 			HostIP = pods.Items[rand.Intn(len(pods.Items))].Status.HostIP
// 			log.Printf("随机选取有该Pod的Ip %s", HostIP)
// 		}
		
// 		baseURL := proxy.NodeNetesURL[HostIP]


// 		log.Printf("被转发到BaseUrl %s\n", baseURL)
// 		log.Printf("被转发到requestUrl %s\n", requestURL)
