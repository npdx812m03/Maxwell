...
func main() {
	...
	pods, err := clientset.CoreV1().Pods(config.CNamespace).List(context.TODO(), metav1.ListOptions{LabelSelector: "app=node-netes"})
	
	if err != nil {
		log.Println(err.Error())
	}
	var nn_IPs = make(map[string]string)

	for _, item := range pods.Items {
		var hostIP = item.Status.HostIP
		var podIP = "http://" + item.Status.PodIP + ":8081"
		nn_IPs[hostIP] = podIP
	}

	reverseProxy := types.NewHTTPClientReverseProxy(
	...
	r.HandleFunc("/chain/{name:["+NameExpression+"]+}", functionProxy)
	r.HandleFunc("/chain/{name:["+NameExpression+"]+}/", functionProxy)
	r.HandleFunc("/chian/{name:["+NameExpression+"]+}/{params:.*}", functionProxy)
