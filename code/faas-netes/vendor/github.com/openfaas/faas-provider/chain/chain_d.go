...
// 	Resolve(functionName string) (url.URL, types.FunctionDockerMetadata, error)
// 	ReturnPod(functionName string, serviceIP string)
// 	GetResource(chainName string, stage int) int
...
// 	functionChain := resolver.GetChain(chainName)
...
// 	for index, functionName := range functionChain{
// 		start := time.Now()
// 		functionAddr, docker, resolveErr := resolver.Resolve(functionName)
// 		resource := resolver.GetResource(chainName, index)
// 		log.Println(resource)
// 		reviseResource(docker, resource, proxyClient)
...
// 		resolver.ReturnPod(functionName, serviceIP)

// 		if index != len(functionChain) -1 {
// 			originalReq = Transformation(response)
// 		}
// 	}
...
// 		httputil.Errorf(w, http.StatusInternalServerError, "Can't reach service for: %s.", chainName)
// 		return
// 	}
