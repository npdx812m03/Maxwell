...
func proxyRequest(w http.ResponseWriter, originalReq *http.Request, proxyClient *http.Client, resolver BaseURLResolver) {
	...
	// 根据请求的函数名查找函数的位置,主要改动！
	functionAddr, _, resolveErr := resolver.Resolve(functionName, 100)
	// log.Println("目标docker ", docker)
	if resolveErr != nil {
	...
	// chain.ReviseResource(docker, 10, proxyClient)
	...
	client.LPush("latencySingle", seconds.Seconds())

	serviceIP := strings.Split(functionAddr.Host, ":")[0]
	returnName := strings.Split(functionName, ".")[0]
	resolver.ReturnPod(returnName, serviceIP, 100)
	...
	res := strconv.FormatFloat(seconds.Seconds(), 'f', -1, 64)
	if response.Body != nil {
		// log.Printf("返回类型为: %T\n", response.Body)
		// log.Printf("返回结果为: %s\n", response.Body)
