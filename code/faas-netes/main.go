...
func startInformers(setup serverSetup, stopCh <-chan struct{}, operator bool) customInformers {
	...
	go nodes.Informer().Run(stopCh)
	if ok := cache.WaitForNamedCacheSync("faas-netes:nodes", stopCh, nodes.Informer().HasSynced); !ok {
		log.Fatalf("failed to wait for cache to sync")
	}
...
func runController(setup serverSetup) {
	...
	bootstrapHandlers := providertypes.FaaSHandlers{
		ChainProxy:			  chain.NewHandlerChain(config.FaaSConfig, functionLookup),
	...
	faasProvider.Serve(&bootstrapHandlers, &config.FaaSConfig, listers.DeploymentInformer.Lister(), listers.NodeInformer.Lister())
}
...
func runOperator(setup serverSetup, cfg config.BootstrapConfig) {
	...
	go srv.Start(listers.DeploymentInformer.Lister(), listers.NodeInformer.Lister())
